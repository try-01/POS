...
/**
 * Dialog full-screen "Kelola Printer" -- pola serupa dialog manajemen
 * multi-item lain di app ini (mis. Kelola Shift di Batch F), dipilih alih-alih
 * AlertDialog biasa karena perlu menampung daftar + banyak aksi per baris.
 */
                    ...
                    if (printers.isEmpty()) {
                        Text(
                            "Belum ada printer ditambahkan. Tambahkan printer WiFi/LAN " +
                                "terlebih dahulu (dukungan Bluetooth & USB menyusul).",
    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                Text(
                    "Bluetooth & USB akan tersedia pada pembaruan berikutnya.",
...
private fun printerSubtitle(printer: PrinterEntity): String = when (printer.connectionType) {
...
private fun PrinterConnectionType.icon() = when (this) {
